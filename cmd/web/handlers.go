...
func (app *application) CreateDogWithBuilder(w http.ResponseWriter, r *http.Request) {
	...
	p, err := pets.NewPetBuilder().
		...
		SetWeight(15).
		SetDescription("a mixed breed or unknown origin. probably has some German Shepherd heritage").
...
func (app *application) CreateCatWithBuilder(w http.ResponseWriter, r *http.Request) {
	...
	p, err := pets.NewPetBuilder().
		...
		SetWeight(5).
		SetDescription("a friendly domestic shorthair cat with a playful personality").
		SetColor("Gray Tabby").
		SetAge(2).
		SetAgeEstimated(true).
		Build()
